...
# log path
LOG_PATH = "/data/lyh/lab/pcaifeat_RobotCar_v3_1/log/train_save_trans_only_train_fusion"
...
def init_pcainetwork():
	...
	variables = tf.trainable_variables()
	img_train_variable = [v for v in variables if v.name.split('/')[0] =='img_var' and v.name.split('/')[1] != 'resnet_v1_50']
	fusion_variable = [v for v in variables if v.name.split('/')[0] =='fusion_var']
	only_fusion_training_var = fusion_variable + img_train_variable
	

	if ONLY_TRAIN_FUSION and TRAINING_MODE == 3:
		with tf.control_dependencies(update_ops):
			fusion_train_op = optimizer.minimize(all_loss, global_step=step,var_list=only_fusion_training_var)
